...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((sum, char) => sum + Number(char.charCodeAt(0)), 0);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .reduce((prev, cur) => (vowels.includes(cur) ? prev + 1 : prev), 0);
}
...
function isPalindrome(str) {
  const letter = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
  const head = letter.substring(0, Math.floor(letter.length / 2));
  const tail = letter
    .substring(Math.ceil(letter.length / 2))
    .split('')
    .reverse()
    .join('');
  return head === tail;
...
function findLongestWord(sentence) {
  ...
  const maxLength = lengths.reduce((prev, cur) => (prev < cur ? cur : prev), 0);
  return arr[lengths.indexOf(maxLength)];
}
...
function invertCase(str) {
  return str
    .split('')
    .map((letter) =>
      letter.codePointAt() < 97 ? letter.toLowerCase() : letter.toUpperCase()
    )
    .join('');
}
...
function encodeToRot13(str) {
  const arr = str.split('').map((letter) => {
    ...
    const letterUp =
      (codePoint >= 65 && codePoint <= 77) ||
      (codePoint >= 97 && codePoint <= 109);
    const letterDown =
      (codePoint >= 78 && codePoint <= 90) ||
      (codePoint >= 110 && codePoint <= 122);

    return codePoint + (letterUp && 13) + (letterDown && -13);
  });
...
function getCardId(value) {
  ...
  const row = m.indexOf(value.slice(-1)) * 13;
  const col = a.indexOf(number);
  return row + col;
}
